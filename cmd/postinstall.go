...
const skillURL = "https://raw.githubusercontent.com/Cloverhound/five9-cli/main/skill/SKILL.md"
...
var agentPlatforms = []agentPlatform{
	{"Claude Code", filepath.Join(".claude", "skills", "five9-cli")},
	{coworkName, ""}, // Cowork requires ZIP upload via web UI
	{"OpenAI Codex", filepath.Join(".codex", "skills", "five9-cli")},
	{"Cursor", filepath.Join(".cursor", "skills", "five9-cli")},
}
	...
	form := huh.NewForm(
		huh.NewGroup(
			huh.NewSelect[string]().
				...
				Description(
					fmt.Sprintf(
						"The five9 binary was installed to %s, but your shell\n"+
							"can't find it yet. Adding it to %s will make the\n"+
							"\"five9\" command available in all new terminal sessions.",
...
func setupWindowsPath(installDir string) error {
	fmt.Printf("Add %s to your system PATH to use the \"five9\" command.\n", installDir)
	return nil
}
...
func setupAgentSkills() error {
	...
	// Build options, pre-selecting platforms that are detected on the system
	...
	form := huh.NewForm(
		huh.NewGroup(
			huh.NewMultiSelect[string]().
				Title("Install Five9 skill for AI coding agents?").
				Description(
					"The Five9 skill lets AI coding agents (Claude Code, Codex, Cursor)\n"+
						"query and manage your Five9 environment using natural language.\n"+
	...
	// Download the skill file once
	fmt.Print("Downloading Five9 skill...")
	...
	for _, name := range selected {
		for _, p := range agentPlatforms {
			...
			if p.Name == coworkName {
				zipPath := filepath.Join(home, "Downloads", "five9-cli-skill.zip")
				if err := buildSkillZip(zipPath, "five9-cli", skillContent); err != nil {
...
// agentDetected checks if the agent platform's config directory exists.
func agentDetected(home string, p agentPlatform) bool {
	if p.Name == coworkName {
		// Detect Claude Desktop app (which includes Cowork)
		switch runtime.GOOS {
		case "darwin":
	...
	// Check for the agent's top-level config dir (e.g. ~/.claude, ~/.cursor)
